...
sudo mkdir /etc/opt/clairv4/config/
----
...
.Configure Clair
----
sudo cat <<EOF > /etc/opt/clairv4/config/config.yaml
http_listen_addr: :9999
introspection_addr: :9998
log_level: debug
indexer:
  connstring: host=localhost port=5433 dbname=clair user=clairuser password=clairpass sslmode=disable
  scanlock_retry: 10
